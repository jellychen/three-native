...
void GLRenderer::setRenderTarget(GLRenderTarget* target) {
    if (target) {
        activeTarget = target;
        target->bind();
        glViewport(0, 0, target->options.width, target->options.height);
    } else {
        activeTarget = nullptr;
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
    }
}
void GLRenderer::renderFullScreenQuad() {
    ensureFSQuad();
    glBindVertexArray(fsQuadVAO);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
    glBindVertexArray(0);
}
void GLRenderer::ensureFSQuad() {
    if (fsQuadVAO) return;
...
void GLRenderer::render(Scene& scene, Camera& camera) {
    ...
    // If a render target is active, bind to it instead of the default framebuffer.
    if (activeTarget) {
        activeTarget->bind();
        glViewport(0, 0, activeTarget->options.width, activeTarget->options.height);
    } else {
#if !THREECPP_USE_ANGLE
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
